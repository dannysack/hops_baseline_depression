...
# first need to create tidy dataframe with 6 colums: term, estimate, std.error, statistic, p.value, and nobs (degrees of freedom)
...
# how to get it to work with fit.mult.impute in Hmisc.5.0-2...will not redo all code...
# need to set cluster to one imputed dataset instead of base dataset because of changes in the order
# after splitting and re-joining imputations as done in this case to reduce effect modification
# test2 <- fit.mult.impute(formula = f_ua_form6, orm, imp_fem, data = fem_fin, cluster = complete(imp_fem, 1)$clinic.female)
...
prim_cc6_f <- complete(imp_fem, "all") %>%
  map(~ robcov(
      orm(f_a_form6, data = .x %>% filter(!is.na(phq9_6m_cat.female)), x = TRUE, y = TRUE),
...
prim_cc18_f <- complete(imp_fem, "all") %>%
  map(~ robcov(
      orm(f_a_form18, data = .x %>% filter(!is.na(phq9_18m_cat.female)), x = TRUE, y = TRUE),
...
prim_cc6_m <- complete(imp_m, "all") %>%
  map(~ robcov(
      orm(m_a_form6, data = .x %>% filter(!is.na(phq9_6m_cat.male)), x = TRUE, y = TRUE),
...
prim_cc18_m <- complete(imp_m, "all") %>%
  map(~ robcov(
      orm(m_a_form18, data = .x %>% filter(!is.na(phq9_18m_cat.male)), x = TRUE, y = TRUE),
...
# 6 months
prim_int6_f <- fit.mult.impute(formula = phq9_6m.female ~ group.female*rcs(socsup.female, 3) + 
                                 rcs(day.female, 3) + rcs(age.female, 3) + who.female + 
                                 edu_cat.female + rcs(stigma.female, 3) + rcs(phq9_b.male, 3), orm,
                               xtrans = imp_fem, data = fem_fin, cluster = complete(imp_fem, 1)$clinic.female)
...
# 18 months
prim_int18_f <- fit.mult.impute(formula = phq9_18m.female ~ group.female*rcs(socsup.female, 3) + 
                                 rcs(day.female, 3) + rcs(age.female, 3) + who.female + 
                                 edu_cat.female + rcs(stigma.female, 3) + rcs(phq9_b.male, 3), orm,
                               xtrans = imp_fem, data = fem_fin, cluster = complete(imp_fem, 1)$clinic.female)
...
ggarrange(f6_pp, f18_pp, common.legend = TRUE)
```
...
```{r int_fem}
# 6 months
prim_int6_m <- fit.mult.impute(formula = phq9_6m.male ~ group.male*rcs(socsup.male, 3) + 
                                 rcs(day.male, 3) + rcs(age.male, 3) + who.male + 
                                 edu_cat.male + rcs(stigma.male, 3) + rcs(phq9_b.female, 3), orm,
                               xtrans = imp_m, data = m_fin, cluster = complete(imp_m, 1)$clinic.male)
...
# 18 months
prim_int18_m <- fit.mult.impute(formula = phq9_18m.male ~ group.male*rcs(socsup.male, 3) + 
                                 rcs(day.male, 3) + rcs(age.male, 3) + who.male + 
                                 edu_cat.male + rcs(stigma.male, 3) + rcs(phq9_b.female, 3), orm,
                               xtrans = imp_m, data = m_fin, cluster = complete(imp_m, 1)$clinic.male)
...
ggarrange(m6_pp, m18_pp, common.legend = TRUE)
```
